    ...
    <main className="flex-1 px-6 py-8">
      <div className="max-w-5xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <div>
            <p className="text-sm text-slate-400">你好，{self.name}</p>
            <h2 className="text-2xl font-semibold">大厅</h2>
          </div>
          <div className="flex gap-2">
            <input
              ...
              placeholder="房间名称"
              className="px-3 py-2 rounded-lg bg-slate-800 border border-white/5 text-sm focus:border-accent outline-none"
            />
            <button
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {rooms.map((room) => (
            ...
            >
              <div>
                ...
                <div className="flex flex-wrap gap-2 mt-3">
                  {room.players.map((p) => (
                    <span
                      key={p.id}
                      className="text-xs px-2 py-1 rounded bg-white/5 border border-white/5"
                      style={{ color: p.color }}
                    >
                      {p.name}
                    </span>
              ...
              <button
                className="mt-4 px-3 py-2 rounded-lg bg-white/10 text-sm font-semibold hover:bg-white/15"
